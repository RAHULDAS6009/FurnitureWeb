...
const filterProducts = (list, q, cat) => {
  ...
  return list.filter(p => {
    const inCat = category === "all" ? true : normalize(p.category || "") === category || normalize(p.title).includes(category);
    const matchesQ = !query ? true : (
...
const SearchResults = () => {
  ...
  const results = useMemo(() => filterProducts(products2, q, cat), [q, cat]);
  ...
  return (
    ...
    <div className="ul-container py-10">
      <div className="ul-section-heading flex items-center justify-between">
        <div>
          ...
          <h2 className="ul-section-title">
            Results {q ? <>for “{q}”</> : null} {cat && cat !== "all" ? <>in “{cat}”</> : null}
          </h2>
        </div>
      ...
      {results.length === 0 ? (
        <p className="text-sm opacity-70">No products found. Try different keywords or category.</p>
      ) : (
              ...
              <div className="ul-product-horizontal">
                ...
                <div className="ul-product-horizontal-txt">
                  ...
                  <h4 className="ul-product-title">
                    <a href={`/shopdetails/${p.id}`} onClick={(e)=>e.preventDefault()}>{p.title}</a>
                  </h4>
                  <h5 className="ul-product-category">
                    <a href="/shop" onClick={(e)=>e.preventDefault()}>{p.category}</a>
                  </h5>
